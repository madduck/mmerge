...
@clickx.config_option(  # type: ignore[untyped-decorator]
    ...
    default=pathlib.Path(pathlib.Path.cwd() / "mmerge.toml"),
)
...
for plugin in PLUGINS:
    try:
        mmerge.add_command(plugin)  # type: ignore[arg-type]
        ...
        try:
            ...
            mmerge.add_command(mod.plugincmd, name=plugin)

    logger.debug(f"Added plugin to mmerge: {plugin}")
        ...
        try:
            async with asyncio.TaskGroup() as tg:
                ...
                try:
                    ...
                    srctasks = set()
                    async with asyncio.TaskGroup() as tgsrc:
                    ...
                    total = sum(t.result() for t in srctasks)
